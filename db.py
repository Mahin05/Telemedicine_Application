...
def save_user(username, email, password, contact, dob,gender, profile_pic):
    password_hash = generate_password_hash(password)
    user_data_collection.insert_one(
        {'_id': username, 'email': email, 'password': password_hash, 'contact': contact,  'dob': dob,'gender': gender,
         'profile_pic': profile_pic})


def update_user(username, email, contact, gender, dob):
    user_data_collection.update_many({'_id': username},
                                     {'$set': {'email': email, 'contact': contact, 'gender': gender, 'dob': dob}})
...
def send_appointment(username, doctors, diesease, contact, datetime, subject):
    appointments_data_collection.insert_one(
        {'_id': username, 'doctors': doctors, 'diesease': diesease, 'contact': contact, 'datetime': datetime,
         'subject': subject})


def get_user(username):
    user_data = user_data_collection.find_one({'_id': username})
    return User(user_data['_id'], user_data['email'], user_data['password'], user_data['contact'],
                user_data['dob'], user_data['gender'],user_data['profile_pic']) if user_data else None
def get_appointment(username):
    appointment_data = appointments_data_collection.find_one({'_id': username})
    return Appointment(appointment_data['_id'], appointment_data['doctors'], appointment_data['diesease'],appointment_data['contact'] ,appointment_data['datetime'],appointment_data['subject']) if appointment_data else None
